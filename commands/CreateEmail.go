...
func (c *CreateEmailCommand) ExecuteDash(session *discordgo.Session, messageCreate *discordgo.MessageCreate, args string) {
	matches := createEmailRegex.FindStringSubmatch(strings.TrimSpace(args))
	if len(matches) == 0 {
		_, _ = session.ChannelMessageSendReply(messageCreate.ChannelID, "Please provide valid arguments", messageCreate.Reference())
		return
	}
	domain := matches[createEmailRegex.SubexpIndex("domain")]
	localPart := matches[createEmailRegex.SubexpIndex("localPart")]
	password := matches[createEmailRegex.SubexpIndex("password")]
	if password == "" {
